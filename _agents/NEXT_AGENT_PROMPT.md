...
- Contains 310+ source files, 100+ React components, 515 passing Vitest tests across 26 test files
...
8. **Test infrastructure**: 515 Vitest tests across 26 test files (data integrity, accessibility, i18n, profiles, sanctions, source links, CCP influence detection, timeline, sitemap, security)
...
npx vitest run    # Should show 515 tests passing across 26 test files
```
...
### Test Commands
```bash
npx vitest run                           # All 515 tests (26 files)
...
**Repository state:** 515 tests passing, build clean, terminal design 100% applied, 12 profiles, 8 languages, 35 sanctions, 31 timeline events, 0 orphan components  
