...
> Last Updated: March 6, 2026 (Session 215)
>
...
- [x] **Advocacy Letter Generator (Session 204)**: New `src/components/AdvocacyLetterGenerator.jsx` — data-driven personalized advocacy letters for all 63 documented political prisoners. Uses dataApi to pull real case data (prisoner name, sentence, location, health status, international response). Features: prisoner selector with search/filter, 5 target countries (US/UK/CA/AU/EU), auto-generated letters with case-specific details, copy-to-clipboard. Terminal palette compliant. Integrated into Take Action page (lazy-loaded, after Contact Representatives). AdvocacyLetterGenerator.test.jsx (36 tests). Added recent_updates.json entry (37 entries total). 2606 tests (161 files, all passing).
- [x] **Case Timeline Viewer (Session 205)**: New `src/components/CaseTimelineViewer.jsx` — cross-references 63 political prisoners with 34 timeline events using keyword matching. Features: searchable prisoner selector, prisoner info card (status/sentence/location/health), chronological vertical timeline with category badges (HK/Uyghur/Tibet/Mainland/Falun Gong/Global), significance indicators, copy-to-clipboard. Uses CASE_KEYWORDS map for 12 key prisoners + fallback name matching. Terminal palette compliant (orange for falungong, no rounded-lg). Integrated into Political Prisoners page (lazy-loaded, after Case Studies, before Memorial Wall). CaseTimelineViewer.test.jsx (37 tests). Added recent_updates.json entry (38 entries total). 2643 tests (162 files, all passing).
- [x] **International Response Tracker (Session 206)**: New `src/components/InternationalResponseTracker.jsx` — visualises how 30 countries have responded to CCP human rights violations across 5 dimensions: genocide recognition, sanctions imposed, legislative actions, diplomatic actions, pending proposals. Features: stance classification (Strong/Moderate/Limited/Weak) with colour-coded summary bar, country search, stance filter buttons with aria-pressed, expandable country detail cards, strength bars, source links, copy-to-clipboard. Added `getInternationalResponses()`, `getInternationalResponseByCountry()`, `searchInternationalResponses()` to dataApi.js. Updated `getDatasetSummary()` (9 datasets) and `globalSearch()` to include international_responses. Integrated into Take Action page (lazy-loaded, after Sanctions Tracker). InternationalResponseTracker.test.jsx (43 tests). Added recent_updates.json entry (39 entries total). 2686 tests (163 files, all passing).
- [x] **Sanction Impact Analyzer (Session 207)**: New `src/components/SanctionImpactAnalyzer.jsx` — cross-dataset analysis of international sanctions against 34 CCP officials from 47 sanctions across 5 countries (US/UK/EU/Canada/Australia). Features: summary stats (officials/sanctions/countries), country breakdown bars (sanctions per country), coverage classification (Full/Strong/Partial/Gap) with filter buttons, search by name/position, expandable official details (position, key abuses, per-country sanction status grid with flags, advocacy opportunity alerts for coverage gaps, current status, source links), copy-to-clipboard with CC BY 4.0 attribution. Terminal palette compliant. Integrated into Intelligence Feeds → CCP Operations tab (lazy-loaded, after Sanctioned Officials). SanctionImpactAnalyzer.test.jsx (46 tests). Added recent_updates.json entry (40 entries total). 2732 tests (164 files, all passing).
- [x] **Human Rights Org Directory (Session 208)**: New `src/components/HumanRightsOrgDirectory.jsx` — interactive directory of 49 verified human rights organizations working on China issues. Features: filter by focus area (Uyghur/Tibet/HK/General China/Legal/Media/Research/Taiwan) with colour-coded badges, filter by type (Advocacy/Research/Legal/Media/Coalition/Direct Support), search by name/headquarters/key work, expandable org cards with key work, latest news, credibility indicators (High shield badge), website/donation/source links, copy-to-clipboard with CC BY 4.0 attribution. Added `getHumanRightsOrgs()`, `getHumanRightsOrgsByFocus()`, `getHumanRightsOrgsByType()`, `searchHumanRightsOrgs()` to dataApi.js. Updated `getDatasetSummary()` (10 datasets) and `globalSearch()` to include human_rights_orgs. Terminal palette compliant. Integrated into Education Center → Research tab (lazy-loaded, after Academic Experts). HumanRightsOrgDirectory.test.jsx (43 tests). Added recent_updates.json entry (41 entries total). 2775 tests (165 files, all passing).
- [x] **Overseas Police Station Tracker (Session 209)**: New `src/components/OverseasPoliceStationTracker.jsx` — interactive tracker of 30 CCP overseas police service stations across 27 countries used for transnational repression. Features: status summary cards (Closed 7 / Under Investigation 16 / Operating 4 / Unknown 3) with colour-coded status bar, status filter buttons with aria-pressed, search by country/city/government response, expandable station cards with government response, arrest details, linked provinces, latest news, source links, copy-to-clipboard with CC BY 4.0 attribution. Added `getPoliceStations()`, `getPoliceStationsByCountry()`, `getPoliceStationsByStatus()`, `searchPoliceStations()` to dataApi.js. Updated `getDatasetSummary()` (11 datasets) and `globalSearch()` to include police_stations. Terminal palette compliant. Integrated into Intelligence Feeds → CCP Operations tab (lazy-loaded, after Sanction Impact Analyzer). OverseasPoliceStationTracker.test.jsx (44 tests). Added recent_updates.json entry (42 entries total). 2819 tests (166 files, all passing).
