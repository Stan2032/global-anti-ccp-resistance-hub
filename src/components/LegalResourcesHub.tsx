    ...
    <div className="bg-[#111820]/50 backdrop-blur-sm border border-[#1c2a35]/50 p-6">
      ...
      <div className="space-y-4 mb-6">
        <div className="relative">
          ...
          <input
            aria-label="Search legal resources by country, topic, or keyword"
            type="text"
            placeholder="Search legal resources by country, topic, or keyword..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-10 pr-4 py-2 bg-[#0a0e14]/50 border border-[#1c2a35]/50 text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#22d3ee]/50"
          />
        </div>
                    ...
                    <div key={cIndex} className="bg-[#111820]/50 rounded p-3">
                      ...
                      {contact.phone && (
                        <a href={`tel:${contact.phone}`} className="block text-red-400 text-sm font-mono mb-1">
                          <span className="flex items-center gap-1"><Phone className="w-3 h-3 flex-shrink-0" /> {contact.phone}</span>
                        </a>
                      )}
                      {contact.email && (
                        <div className="text-[#22d3ee] text-sm font-mono break-all">
                          <span className="flex items-center gap-1"><Mail className="w-3 h-3 flex-shrink-0" /> {contact.email}</span>
                        </div>
                      )}
                    </div>
                  ))}
