...
describe('Cross-JSON Data Consistency', () => {
  let prisoners, alerts, sanctions, officials, recentUpdates, forcedLabor, detentionFacilities;
  ...
  it('prisoners referenced in alert IDs exist in political_prisoners_research.json', () => {
    const prisonerNames = prisoners.results.map(r => r.output.prisoner_name.toLowerCase());
  ...
  it('active critical alerts have lastVerified within 60 days', () => {
    ...
    for (const alert of alerts.filter(a => a.active && a.type === 'critical')) {
      if (alert.lastVerified) {
        ...
        const daysSince = (now - verifiedDate) / (24 * 60 * 60 * 1000);
        expect(
          daysSince,
  ...
  it('sanctioned officials have entries that appear in sanctions tracker targets', () => {
    const sanctionTargets = sanctions.sanctions.map(s => s.target.toLowerCase());
    // At least some sanctioned officials should appear in the sanctions tracker
    const officialNames = officials.results.map(r => r.output.name.toLowerCase());
    const overlap = officialNames.filter(name =>
      sanctionTargets.some(target => target.includes(name) || name.includes(target))
    );
    expect(
      overlap.length,
