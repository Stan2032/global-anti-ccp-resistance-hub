...
describe('Route Integrity', () => {
  ...
  it('App defines at least 20 routes', () => {
  ...
  it('all defined routes start with /', () => {
    for (const path of routePaths) {
      if (path === '*') continue;
      expect((path as string).startsWith('/'), `Route "${path}" should start with /`).toBe(true);
    }
  });
  ...
  describe('profile routes integrity', () => {
    it('has at least 15 profile routes', () => {
      const profileRoutes = [...validRoutes].filter(r => (r as string).startsWith('/profiles/'));
    ...
    it('all profile routes use lowercase slugs', () => {
      ...
      for (const route of profileRoutes) {
        const slug = (route as string).replace('/profiles/', '');
